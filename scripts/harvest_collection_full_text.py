...
for itemId in itemIds:
    ...
    itemStore['creator'] = item['Creator'][0]['value'].encode('utf8')
    if 'FullText' in item:
...
with open('full-text.csv', 'w+b') as csvfile:
    ...
    writer.writerow(['ID', 'Title', 'Creator', 'Full Text'])
    for item in items:
        writer.writerow([item['id'], item['title'], item['creator'], item['fullText']])
